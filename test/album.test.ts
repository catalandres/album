...
import { Extended } from "../src/metadata/metadataType";
import * as SalesforceMetadata from '../src/metadata/salesforceMetadata';
...
describe('Album', () => {
    ...
    beforeAll(() => {
        ...
        fsx.copySync('./test/mocks/sfdx-project-mock', folderName);
    });
    ...
    test('parses ApexClass files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.apexClasses.length).toBe(1);

        const apexClass = album.metadata.apexClasses[0];
    ...
    test('parses ApexComponent files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.apexComponents.length).toBe(1);

        const apexComponent = album.metadata.apexComponents[0];
    ...
    test('parses ApexPage files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.apexPages.length).toBe(1);

        const apexPage = album.metadata.apexPages[0];
    ...
    test('parses ApexTrigger files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.apexTriggers.length).toBe(1);

        const apexTrigger = album.metadata.apexTriggers[0];
    ...
    test('parses AuraApplication files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.auraApplications.length).toBe(1);

        const auraApplication = album.metadata.auraApplications[0];
    ...
    test('parses AuraComponent files properly', () => {
        ...
        expect(album.metadata.auraComponents.length).toBe(1);

        const auraComponent = album.metadata.auraComponents[0];
    ...
    test('parses AuraEvent files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.auraEvents.length).toBe(1);

        const auraEvent = album.metadata.auraEvents[0];
    ...
    test('parses AuraInterface files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.auraInterfaces.length).toBe(1);

        const auraInterface = album.metadata.auraInterfaces[0];
    ...
    test('parses AuraTokens files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.auraTokens.length).toBe(1);

        const auraTokens = album.metadata.auraTokens[0];
    ...
    test('parses CompactLayout files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.compactLayouts.length).toBe(1);

        const compactLayout = album.metadata.compactLayouts[0];// as Extended<SalesforceMetadata.CompactLayout>;
    ...
    test('parses CustomField (checkbox) files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.fields.length).toBe(1);

        const customField = album.metadata.fields[0]; // as Extended<SalesforceMetadata.CustomField>;
    ...
    test('parses CustomObject (standard) files properly', () => {
        ...
        const album = new Album([ fileName ]);
        expect(album.metadata.objects.length).toBe(1);

        const customObject = album.metadata.objects[0] as Extended<SalesforceMetadata.CustomObject>;

        console.log(customObject);

        expect(Object.keys(customObject).length).toBe(12);
        ...
        expect(customObject.type).toBe("Standard Object");
    ...
    test('parses CustomObject (custom) files properly', () => {
        ...
        const customObject = album.metadata.objects[0] as Extended<SalesforceMetadata.CustomObject>;
        expect(Object.keys(customObject).length).toBe(27);
        ...
        expect(customObject.type).toBe("Custom Object");
        expect(customObject.xmlns).toBe('http://soap.sforce.com/2006/04/metadata');
        expect(customObject.fileName).toBe(fileName);
